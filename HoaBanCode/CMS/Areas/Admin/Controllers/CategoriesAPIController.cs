...
namespace CMS.Areas.Admin.Controllers
{
    public class CategoriesAPIController : ApiController
    {
        ...
        //GET: API/CategoriesAPI?att=...&&value=...
        public IQueryable<Category> GetCaterory(string att, string value)
        {
            var caterory = db.Category;

            if (att == "idCategoryParent" && att != null && value != null)
            {
                int idCategory = int.Parse(value);
                var model = db.Category.Where(p => p.idCategoryParent == idCategory);
